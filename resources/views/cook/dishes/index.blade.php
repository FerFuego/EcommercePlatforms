...
<div class="container mx-auto px-4 py-12">
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route('cook.dishes.create') }}" class="bg-gradient-to-r from-orange-500 to-pink-600 text-white px-6 py-3 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all">
            ➕ Nuevo Plato
        </a>
    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($dishes as $dish)
                <div class="bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all group">
                    ...
                    <div class="relative">
                        @if($dish->photo_url)
                            <img src="{{ Storage::url($dish->photo_url) }}" 
                                 alt="{{ $dish->name }}" 
                                 class="w-full h-48 object-cover group-hover:scale-105 transition-transform">
                        @else
                            <div class="w-full h-48 bg-gradient-to-br from-orange-300 to-pink-400 flex items-center justify-center text-6xl">
                                🍲
                            </div>
                        @endif
                    ...
                    <div class="p-6">
                            ...
                            <div class="text-right">
                                ...
                                <div class="flex items-center space-x-2">
                                    <button onclick="updateStock({{ $dish->id }}, -1, {{ $dish->available_stock }})" 
                                            class="w-8 h-8 bg-gray-200 rounded-lg hover:bg-gray-300 transition">-</button>
                                    <span id="stock-{{ $dish->id }}" class="font-bold text-lg w-12 text-center">{{ $dish->available_stock }}</span>
                                    <button onclick="updateStock({{ $dish->id }}, 1, {{ $dish->available_stock }})" 
                                            class="w-8 h-8 bg-gray-200 rounded-lg hover:bg-gray-300 transition">+</button>
                                </div>
                            </div>
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <label class="flex items-center justify-center p-2 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100 transition">
                                <input type="checkbox" 
                                       {{ $dish->is_active ? 'checked' : '' }}
                                       onchange="toggleActive({{ $dish->id }})"
                        ...
                        <form action="{{ route('cook.dishes.destroy', $dish->id) }}" method="POST" class="mt-2" onsubmit="return confirm('¿Seguro que deseas eliminar este plato?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="w-full p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition text-sm font-medium">
                                🗑️ Eliminar
                            </button>
                        </form>
                    </div>
    ...
    @else
        <div class="bg-white rounded-2xl shadow-lg p-12 text-center">
            ...
            <a href="{{ route('cook.dishes.create') }}" class="inline-block bg-gradient-to-r from-orange-500 via-pink-500 to-purple-600 text-white px-8 py-4 rounded-xl font-bold shadow-lg hover:shadow-xl transition-all">
