            ...
            <div class="flex justify-between items-center h-20 gap-3 lg:gap-6">
                ...
                <div class="hidden md:flex items-center space-x-2.5 lg:space-x-3.5 flex-shrink-0">
                    ...
                    <div class="flex items-center space-x-1 lg:space-x-1.5 text-gray-700">
                        ...
                        <a href="{{ route('cart.index') }}"
                            ...
                            <span
                                class="header-cart-badge absolute -top-0.5 -right-0.5 bg-gradient-to-r from-orange-500 to-pink-500 text-white text-[11px] font-bold rounded-full min-w-[18px] h-[18px] px-1 {{ $cartCount > 0 ? 'flex' : 'hidden' }} items-center justify-center shadow-sm">
                        ...
                        @auth
                            ...
                            @if(auth()->user()->isCustomer() || auth()->user()->orders()->exists())
                                <a href="{{ route('orders.my') }}"
                                    ...
                                    @if($pendingCount > 0)
                                        <span
                                            class="absolute -top-0.5 -right-0.5 bg-gradient-to-r from-orange-500 to-pink-500 text-white text-[11px] font-bold rounded-full min-w-[18px] h-[18px] px-1 flex items-center justify-center shadow-sm">
                                            {{ $pendingCount }}
                                        </span>
                                    @endif
                                </a>
                        ...
                        <a href="{{ $globalSettings['instagram_url'] ?? 'https://www.instagram.com/cocinarte.ar' }}"
                            ...
                            <svg class="w-5 h-5 fill-currentColor transition-transform group-hover:scale-110"
                                viewBox="0 0 24 24">
                                <path
                        ...
                        @auth
                            ...
                            <div class="relative group ml-1">
                                <button
                                    ...
                                    @else
                                        <div
                                            class="w-9 h-9 bg-gradient-to-br from-purple-500 to-pink-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm">
                                            {{ substr(auth()->user()->name, 0, 1) }}
                                        </div>
                                    @endif
                                </button>
                                <div
                                    class="absolute right-0 mt-2 w-52 bg-white rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 border border-gray-100">
                                    <div class="py-2">
                ...
                <div class="flex items-center space-x-2 md:hidden">
                    ...
                    <a href="{{ route('cart.index') }}"
                        ...
                        <span
                            class="header-cart-badge absolute top-0 right-0 bg-gradient-to-r from-orange-500 to-pink-500 text-white text-[11px] font-bold rounded-full min-w-[18px] h-[18px] px-1 {{ $cartCount > 0 ? 'flex' : 'hidden' }} items-center justify-center shadow-sm">
                            <span class="header-cart-count">{{ $cartCount }}</span>
                        </span>
                    </a>
            ...
            <div class="px-4 pt-3 pb-6 space-y-2">
                ...
                @if(!$isAuthPage)
                    <form action="{{ route('marketplace.catalog') }}" method="GET" class="mb-3">
                        <div
                            class="relative flex items-center w-full bg-gray-50 rounded-full border border-gray-200 focus-within:border-orange-500 focus-within:bg-white shadow-sm">
                            ...
                            <button type="submit"
                                class="absolute right-1.5 w-7 h-7 rounded-full bg-gradient-to-r from-orange-500 to-pink-500 text-white flex items-center justify-center shadow-sm"
                                aria-label="Buscar">
                ...
                @auth
                    ...
                    @if(auth()->user()->isCustomer() || auth()->user()->orders()->exists())
                        <a href="{{ route('orders.my') }}"
                            ...
                            @if($pendingCount > 0)
                                <span
                                    class="bg-gradient-to-r from-orange-500 to-pink-500 text-white text-xs rounded-full px-2 py-0.5">{{ $pendingCount }}</span>
                            @endif
                        </a>
                    @endif
                    ...
                    <a href="{{ route('cart.index') }}"
                        ...
                        @if($cartCount > 0)
                            <span
                                class="bg-gradient-to-r from-pink-500 to-purple-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{{ $cartCount }}</span>
                        @endif
                    </a>
