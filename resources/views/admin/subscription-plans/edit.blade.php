...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-200">Editar Plan de Suscripción</h2>
            ...
            <form action="{{ route('admin.subscription-plans.update', $subscriptionPlan) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select id="billing_period" name="billing_period"
                            ...
                            <option value="monthly" {{ old('billing_period', $subscriptionPlan->billing_period) == 'monthly' ? 'selected' : '' }}>Mensual</option>
                            <option value="yearly" {{ old('billing_period', $subscriptionPlan->billing_period) == 'yearly' ? 'selected' : '' }}>Anual</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="number" name="monthly_sales_limit" id="monthly_sales_limit" step="0.01"
                            value="{{ old('monthly_sales_limit', $subscriptionPlan->monthly_sales_limit) }}"
                            class="mt-1 flex-1 block w-full rounded-md sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                    ...
                    <div>
                        ...
                        <input type="number" name="monthly_orders_limit" id="monthly_orders_limit"
                            value="{{ old('monthly_orders_limit', $subscriptionPlan->monthly_orders_limit) }}"
                            class="mt-1 flex-1 block w-full rounded-md sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                        <span class="text-xs text-gray-500">Dejar en blanco para ilimitado</span>
                ...
                <div class="mb-4">
                    ...
                    <div class="mt-1 relative rounded-md shadow-sm">
                        <input type="number" name="commission_percentage" id="commission_percentage" step="0.01"
                            value="{{ old('commission_percentage', $subscriptionPlan->commission_percentage) }}" required
                            class="mt-1 flex-1 block w-full rounded-md sm:text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white">
                    </div>
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="feature_badge" name="features[premium_badge]" type="checkbox"
                                    class="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded" {{ old('features.premium_badge', $features['premium_badge'] ?? false) ? 'checked' : '' }}>
                            </div>
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="feature_offers" name="features[can_create_offers]" type="checkbox"
                                    class="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded" {{ old('features.can_create_offers', $features['can_create_offers'] ?? false) ? 'checked' : '' }}>
                            </div>
                            <div class="ml-3 text-sm">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="feature_stats" name="features[advanced_stats]" type="checkbox"
                                    class="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded" {{ old('features.advanced_stats', $features['advanced_stats'] ?? false) ? 'checked' : '' }}>
                            </div>
                            <div class="ml-3 text-sm">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="feature_priority" name="features[priority_listing]" type="checkbox"
                                    class="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded" {{ old('features.priority_listing', $features['priority_listing'] ?? false) ? 'checked' : '' }}>
                            </div>
                            <div class="ml-3 text-sm">
                ...
                <div class="mb-6 flex items-start">
                    <div class="flex items-center h-5">
                        <input id="is_active" name="is_active" type="checkbox"
                            class="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded" {{ old('is_active', $subscriptionPlan->is_active) ? 'checked' : '' }}>
                    </div>
                    <div class="ml-3 text-sm">
                ...
                <div class="flex justify-end gap-4">
                    ...
                    <button type="submit"
                        class="bg-gradient-to-r from-purple-600 to-pink-600 text-white px-3 py-2 rounded-xl font-bold hover:shadow-lg hover:scale-105 transform transition duration-300">Actualizar
                        Plan</button>
                </div>
            </form>
        </div>
